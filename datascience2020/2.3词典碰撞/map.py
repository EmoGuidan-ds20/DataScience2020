...
for per in file4:#共4个时间段：每次修改file编号和打开的词典编号，以及最后存入文件的编号
    ...
    if index==0:
       ...
       all_list=all_list+list
        ...
        all_list=all_list+list
    index+=1
#得到列表项格式为{"date":xxxx-xx-xx,"text":[]（,"main":xxxxxxx）}（括号内可能存在）
all_list=sorted(all_list,key=itemgetter('date'))
print(all_list)

with open('origin-data/dictionary4.json', encoding='utf8') as fp:#用来碰撞的第x阶段的词典，每次修改字典序号
...
for per in tqdm(all_list):
    ...
    i = 0
    b=[]
    for sentence in per['text']:
        a=[w.word for w in pseg.cut(sentence)]
        b=b+a
...
print(result)
print(len(result))
with open('data/评论4.json', 'w', encoding='utf8') as fp:#每次修改存入文件序号
...
print(first)
with open('data/评论4.1.json', 'w', encoding='utf8') as fp:#每次修改存入文件的序号
